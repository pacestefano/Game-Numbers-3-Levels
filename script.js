...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function startGame() {
        ...
        for (let i = 0; i < 3; i++) {
            createCell(shuffledNumbers[i * 3], `correct-num-${i + 1}`);
            createOperator('+');
            createCell(shuffledNumbers[i * 3 + 1], `correct-num-${i + 1}`);
            createOperator('=');
            createCell(shuffledNumbers[i * 3 + 2], `correct-num-${i + 1}`);
        }
    ...
    function createCell(num, correctClass) {
        let cell = document.createElement('div');
        console.log("Creating cell with number:", num); // Debug
        cell.classList.add('cell', correctClass);
